...
// 更新履歴をJavaScriptで操作
{
    function AddContent (day, content)
    {
        const update_history_contents = document.getElementById('update-history-contents');
        ...
        update_history_content_frame.id =
            `update-history-content-frame-${update_history_contents.childElementCount}`;
        ...
        if (update_history_contents.childElementCount >= 1) {
            update_history_contents.insertBefore(
                update_history_content_li, update_history_contents.firstChild);
        }
        else {
